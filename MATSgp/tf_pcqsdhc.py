...
def cef(x, y, N):
    ...
    M = 2 * N
    M2 = 2 * M
    k = np.arange(-M + 1, M)  # M2 = no. of sampling points.
...
def tf_pcqsdhc(sg0, GamD, Gam0, Gam2, Shift0, Shift2, anuVC, eta, sg):
    ...
    cte = (
        tf.ones_like(sg, dtype=tf.complex128)
        * np.sqrt(np.log(2.0e0))
        / tf.cast(GamD, tf.complex128)
    )
    rpi = np.sqrt(np.pi)
    ...
    c0t = tf.ones_like(sg, dtype=tf.complex128) * (
        1.0e0 - tf.cast(eta, tf.complex128)
    ) * (c0 - 1.5e0 * c2) + tf.cast(anuVC, tf.complex128)
    c2t = (
        tf.ones_like(sg, dtype=tf.complex128)
        * (1.0e0 - tf.cast(eta, tf.complex128))
        * c2
    ...
    if tf.reduce_any(index_PART1):
        ...
        # comment out index_NOT_Z1 becuase not used now
    ...
    if tf.reduce_any(index_PART3):
        ...
        Wb = hum1_wei(
            -tf.math.imag(tf.sqrt(X_TMP)), tf.math.real(tf.sqrt(X_TMP))
        )  # Original just had X, not X_TMP, which would break below (multiplication by Wb)
        Aterm = tf.where(
            index_ABS,
        ...
        Bterm = tf.where(
            index_ABS,
            (1.0e0 / c2t[index_PART3])
            * (
                -1.0e0
                + 2.0e0
                * rpi
                * (1.0e0 - X_TMP - 2.0e0 * Y[index_PART3])
                * (1.0e0 / rpi - tf.sqrt(X_TMP) * (Wb))
    ...
    Aterm_GLOBAL = tf.dynamic_stitch(
        [merge_inds_PART1, merge_inds_PART2, merge_inds_PART3, merge_inds_PART4],
        [Aterm_PART1, Aterm_PART2, Aterm_PART3, Aterm_PART4],
    )
    Bterm_GLOBAL = tf.dynamic_stitch(
        [merge_inds_PART1, merge_inds_PART2, merge_inds_PART3, merge_inds_PART4],
        [Bterm_PART1, Bterm_PART2, Bterm_PART3, Bterm_PART4],
    )

    # common part
    LS_pCqSDHC = (1.0e0 / np.pi) * (
        Aterm_GLOBAL
        / (
            1.0e0
            - (
                tf.cast(anuVC, tf.complex128)
                - tf.cast(eta, tf.complex128) * (c0 - 1.5e0 * c2)
            )
            * Aterm_GLOBAL
            + tf.cast(eta, tf.complex128) * c2 * Bterm_GLOBAL
        )
    )
