...
from .hapi import (
    ISO,
    PYTIPS2017,
    EnvironmentDependency_Intensity,
    molecularMass,
    pcqsdhc,
)
...
def HTP_from_DF_select(
    ...
    for molec in linelist["molec_id"].unique():
        for iso in linelist["local_iso_id"].unique():
            linelist.loc[
                (linelist["molec_id"] == molec) & (linelist["local_iso_id"] == iso),
                "SigmaT",
            ] = PYTIPS2017(molec, iso, T)
            linelist.loc[
                (linelist["molec_id"] == molec) & (linelist["local_iso_id"] == iso),
                "SigmaTref",
            ] = PYTIPS2017(molec, iso, Tref)
            linelist.loc[
                (linelist["molec_id"] == molec) & (linelist["local_iso_id"] == iso), "m"
            ] = (
                molecularMass(molec, iso) * 1.66053873e-27 * 1000
            )  # cmassmol and kg conversion
            if (not natural_abundance) and abundance_ratio_MI != {}:
                linelist.loc[
                    (linelist["molec_id"] == molec) & (linelist["local_iso_id"] == iso),
                    "abun_ratio",
                ] = abundance_ratio_MI[molec][iso]
    linelist["LineIntensity"] = EnvironmentDependency_Intensity(
        linelist["sw"],
        T,
        Tref,
        linelist["SigmaT"],
    ...
    linelist["GammaD"] = (
        np.sqrt(
            2 * 1.380648813e-16 * T * np.log(2) / linelist["m"] / 2.99792458e10 ** 2
        )
        * linelist["nu"]
    )
    ...
    for species in Diluent:
        ...
        linelist["Gamma0"] += abun * (
            linelist["gamma0_%s" % species]
            * (p / pref)
            * ((Tref / T) ** linelist["n_gamma0_%s" % species])
        )
        # Delta0
        linelist["Shift0"] += abun * (
            (
                linelist["delta0_%s" % species]
                + linelist["n_delta0_%s" % species] * (T - Tref)
            )
            * p
            / pref
        )
        # Gamma2
        linelist["Gamma2"] += abun * (
            linelist["SD_gamma_%s" % species]
            * linelist["gamma0_%s" % species]
            * (p / pref)
            * ((Tref / T) ** linelist["n_gamma2_%s" % species])
        )
        # Delta2
        linelist["Shift2"] += abun * (
            (
                linelist["SD_delta_%s" % species] * linelist["delta0_%s" % species]
                + linelist["n_delta2_%s" % species] * (T - Tref)
            )
            * p
            / pref
        )
        # nuVC
        linelist["NuVC"] += abun * (
            linelist["nuVC_%s" % species]
            * (p / pref)
            * ((Tref / T) ** (linelist["n_nuVC_%s" % species]))
        )
        # eta
    ...
    if wing_method == "wing_cutoff":
        linelist["line cut-off"] = (
            0.5346 * linelist["Gamma0"]
            + (0.2166 * linelist["Gamma0"] ** 2 + linelist["GammaD"] ** 2) ** 0.5
        ) * wing_cutoff
    else:
    ...
    for index, line in linelist.iterrows():
        ...
        lineshape_vals_real, lineshape_vals_imag = pcqsdhc(
            ...
            line["NuVC"],
            line["Eta"],
            wavenumbers[BoundIndexLower:BoundIndexUpper],
        )
        Xsect[BoundIndexLower:BoundIndexUpper] += (
            mol_dens
            * molefraction[line["molec_id"]]
            * line["abun_ratio"]
            * line["LineIntensity"]
            * (lineshape_vals_real + line["Y"] * lineshape_vals_imag)
        )
...
class Spectrum:
    ...
    def __init__(
        ...
        frequency_column="Total Frequency /MHz",
    ...
    def calculate_QF(self):
        ...
        """
        return np.around(
            (self.alpha.max() - self.alpha.min()) / self.residuals.std(), 0
        )
    ...
    def fft_spectrum(self):
        ...
        fft_ = (
            fft_[fft_["Amplitude"] > 1e-5]
            .sort_values(["Amplitude"], ascending=[0])
        ...
        for spectrum in self.spectra:
            for segment in list(set(spectrum.segments)):
                ...
                for molecule in spectrum.molefraction:
                    ...
                    ] = spectrum.molefraction[molecule]
                ...
                for etalon_name in spectrum.etalons:
                    line["etalon_" + str(etalon_name) + "_amp"] = spectrum.etalons[
                        etalon_name
                    ][0]
                    line["etalon_" + str(etalon_name) + "_freq"] = spectrum.etalons[
                        etalon_name
                    ][1]
...
def simulate_spectrum(
    ...
    wavenumbers_err = wavenumbers + wave_error * np.random.normal(
        loc=0, scale=1, size=len(wavenumbers)
    )
    # molefraction error
    ...
    for species in molefraction:
        ...
        molefraction_w_error[species] = molefraction[species] + molefraction[
            species
        ] * (molefraction_err[species] / 100)
    # pressure error
    pressure_w_error = pressure_atm + pressure_atm * (
        pressure_err["per_bias"] / 100
    )  # adds the pressure bias based on the percent bias of the pressure measaurement.  Can loop this or set this to be constant for all spectra
    pressure_w_error = len(wavenumbers) * [pressure_w_error]
    if pressure_err["function"] == "linear":
        if "params" in pressure_err:
            pressure_w_error += (
                pressure_err["params"]["m"] * (wavenumbers - np.min(wavenumbers))
                + pressure_err["params"]["b"]
            )
    elif pressure_err["function"] == "sine":
        if "params" in pressure_err:
            pressure_w_error += etalon(
                (wavenumbers - np.min(wavenumbers)),
                pressure_err["params"]["amp"],
                pressure_err["params"]["freq"],
                pressure_err["params"]["phase"],
            )
    # temperature error
    ...
    if temperature_err["function"] == "linear":
        if "params" in pressure_err:
            temperature_w_error += (
                temperature_err["params"]["m"] * (wavenumbers - np.min(wavenumbers))
                + temperature_err["params"]["b"]
            )
    elif pressure_err["function"] == "sine":
        if "params" in pressure_err:
            temperature_w_error += etalon(
                (wavenumbers - np.min(wavenumbers)),
                temperature_err["params"]["amp"],
                temperature_err["params"]["freq"],
                temperature_err["params"]["phase"],
            )
    ...
    for seg in range(0, num_segments):
        ...
        waves, alpha = HTP_from_DF_select(
            parameter_linelist,
            waves,
            wing_cutoff,
            wing_wavenumbers,
        ...
        pressure_array[np.min(segment_array) : np.max(segment_array) + 1] = len(
            alpha
        ) * [segment_pressure]
        temperature_array[np.min(segment_array) : np.max(segment_array) + 1] = len(
            alpha
        ) * [segment_temperature]
    pressure_array = np.asarray(pressure_array) - pressure_atm * (
        pressure_err["per_bias"] / 100
    )
    ...
    if SNR is None:
        ...
        alpha_noise = (
            alpha_array
            + np.max(alpha_array)
            * np.random.normal(loc=0, scale=1, size=len(alpha_array))
            * 1
            / SNR
        )
    ...
    return Spectrum(
        ...
        x_shift=x_shift,
        baseline_order=len(baseline_terms) - 1,
    )
...
class Generate_FitParam_File:
    ...
    def generate_fit_param_linelist_from_linelist(
        ...
        param_linelist_df = param_linelist_df[
            param_linelist_df["sw"] > self.threshold_intensity
        ]  # intensity thresholding
        ...
        param_linelist_df = param_linelist_df[
            param_linelist_df["nu"] < (dataset_max + self.sim_window)
        ]
        param_linelist_df = param_linelist_df[
            param_linelist_df["nu"] > (dataset_min - self.sim_window)
        ]
        ...
        (
            num_nominal_temps,
            list_nominal_temps,
        ) = self.dataset.get_number_nominal_temperatures()
        ...
        for diluent in diluent_list:
            ...
            for nominal_temperature in list_nominal_temps:
                column_list.append(
                    "y_" + diluent + "_" + str(nominal_temperature)
                )  # Fix this so it has the nominal temperatures
        ...
        param_linelist_df["sw_scale_factor"] = [self.fit_intensity] * len(
            param_linelist_df
        )
        ...
        if self.nu_constrain:
            if vary_nu != {}:
                for molecule in vary_nu:
                    for isotope in vary_nu[molecule]:
                        param_linelist_df.loc[
                            (param_linelist_df["nu"] >= dataset_min)
                            & (param_linelist_df["nu"] <= dataset_max)
                            & (param_linelist_df["sw"] > 1)
                            & (param_linelist_df["molec_id"] == molecule)
                            & (param_linelist_df["local_iso_id"] == isotope),
                            "nu_vary",
                        ] = vary_nu[molecule][isotope]
        else:
            for spec in self.dataset.get_list_spectrum_numbers():
                ...
                param_linelist_df["nu_" + str(spec) + "_vary"] = len(
                    param_linelist_df
                ) * [False]
                param_linelist_df["nu_" + str(spec) + "_err"] = len(
                    param_linelist_df
                ) * [0]
                if vary_nu != {}:
                    for molecule in vary_nu:
                ...
                for molecule in vary_sw:
                    for isotope in vary_sw[molecule]:
                        param_linelist_df.loc[
                            ...
                            & (param_linelist_df["local_iso_id"] == isotope),
                            "sw_vary",
                        ] = vary_sw[molecule][isotope]
        else:
            for spec in self.dataset.get_list_spectrum_numbers():
                param_linelist_df["sw_" + str(spec)] = param_linelist_df["sw"].values
                order_sw.append("sw_" + str(spec))
                param_linelist_df["sw_" + str(spec) + "_vary"] = len(
                    param_linelist_df
                ) * [False]
                param_linelist_df["sw_" + str(spec) + "_err"] = len(
                    param_linelist_df
                ) * [0]
                if vary_sw != {}:
                    for molecule in vary_sw:
                        for isotope in vary_sw[molecule]:
                            param_linelist_df.loc[
                                (param_linelist_df["nu"] >= extreme_dictionary[spec][0])
                                & (
                                    param_linelist_df["nu"]
                                    <= extreme_dictionary[spec][1]
                                )
                                & (param_linelist_df["sw"] > 1)
                                & (param_linelist_df["molec_id"] == molecule)
                                & (param_linelist_df["local_iso_id"] == isotope),
        ...
        for diluent in diluent_list:
            ...
            param_linelist_df["gamma0_" + diluent + "_vary"] = len(
                param_linelist_df
            ) * [False]
            param_linelist_df["gamma0_" + diluent + "_err"] = len(param_linelist_df) * [
                0
            ]
                    ...
                    for molecule in vary_gamma0:
                        for isotope in vary_gamma0[molecule]:
                            param_linelist_df.loc[
                                ...
                                & (param_linelist_df["molec_id"] == molecule)
                                & (param_linelist_df["local_iso_id"] == isotope),
                                "gamma0_" + diluent + "_vary",
                            ] = vary_gamma0[molecule][isotope]
            else:
                for spec in self.dataset.get_list_spectrum_numbers():
                    order_gamma0.append("gamma0_" + diluent + "_" + str(spec))
                    param_linelist_df[
                        "gamma0_" + diluent + "_" + str(spec)
                    ] = param_linelist_df["gamma0_" + diluent].values
                    param_linelist_df[
                        "gamma0_" + diluent + "_" + str(spec) + "_vary"
                    ] = len(param_linelist_df) * [False]
                    param_linelist_df[
                        "gamma0_" + diluent + "_" + str(spec) + "_err"
                    ] = len(param_linelist_df) * [0]
                    if vary_gamma0 != {}:
                        for molecule in vary_gamma0:
                            for isotope in vary_gamma0[molecule]:
                                param_linelist_df.loc[
            ...
            param_linelist_df["delta0_" + diluent + "_vary"] = len(
                param_linelist_df
            ) * [False]
            param_linelist_df["delta0_" + diluent + "_err"] = len(param_linelist_df) * [
                0
            ]
                    ...
                    for molecule in vary_delta0:
                        for isotope in vary_delta0[molecule]:
                            param_linelist_df.loc[
                                ...
                                & (param_linelist_df["molec_id"] == molecule)
                                & (param_linelist_df["local_iso_id"] == isotope),
                                "delta0_" + diluent + "_vary",
                            ] = vary_delta0[molecule][isotope]
            else:
                for spec in self.dataset.get_list_spectrum_numbers():
                    order_delta0.append("delta0_" + diluent + "_" + str(spec))
                    param_linelist_df[
                        "delta0_" + diluent + "_" + str(spec)
                    ] = param_linelist_df["delta0_" + diluent].values
                    param_linelist_df[
                        "delta0_" + diluent + "_" + str(spec) + "_vary"
                    ] = len(param_linelist_df) * [0]
                    param_linelist_df[
                        "delta0_" + diluent + "_" + str(spec) + "_err"
                    ] = len(param_linelist_df) * [0]
                    if vary_delta0 != {}:
                        for molecule in vary_delta0:
                            for isotope in vary_delta0[molecule]:
            ...
            param_linelist_df["SD_gamma_" + diluent + "_vary"] = len(
                param_linelist_df
            ) * [False]
            param_linelist_df["SD_gamma_" + diluent + "_err"] = len(
                param_linelist_df
            ) * [0]
                        ...
                        for molecule in vary_aw:
                            for isotope in vary_aw[molecule]:
                                param_linelist_df.loc[
                                    ...
                                    & (param_linelist_df["molec_id"] == molecule)
                                    & (param_linelist_df["local_iso_id"] == isotope),
                                    "SD_gamma_" + diluent + "_vary",
                                ] = vary_aw[molecule][isotope]
            else:
                for spec in self.dataset.get_list_spectrum_numbers():
                    order_SD_gamma.append("SD_gamma_" + diluent + "_" + str(spec))
                    param_linelist_df[
                        "SD_gamma_" + diluent + "_" + str(spec) + "_vary"
                    ] = len(param_linelist_df) * [False]
                    param_linelist_df[
                        "SD_gamma_" + diluent + "_" + str(spec) + "_err"
                    ] = len(param_linelist_df) * [0]
                    if (self.lineprofile == "VP") or (self.lineprofile == "NGP"):
                        param_linelist_df.loc[
                            :, "SD_gamma_" + diluent + "_" + str(spec)
                        ] = 0
                    else:
                        param_linelist_df[
                            "SD_gamma_" + diluent + "_" + str(spec)
                        ] = param_linelist_df["SD_gamma_" + diluent].values
                        if vary_aw != {}:
                            for molecule in vary_aw:
                                for isotope in vary_aw[molecule]:
                                    param_linelist_df.loc[
            ...
            param_linelist_df["SD_delta_" + diluent + "_vary"] = len(
                param_linelist_df
            ) * [False]
            param_linelist_df["SD_delta_" + diluent + "_err"] = len(
                param_linelist_df
            ) * [0]
                        ...
                        for molecule in vary_as:
                            for isotope in vary_as[molecule]:
                                param_linelist_df.loc[
                                    ...
                                    & (param_linelist_df["molec_id"] == molecule)
                                    & (param_linelist_df["local_iso_id"] == isotope),
                                    "SD_delta_" + diluent + "_vary",
                                ] = vary_as[molecule][isotope]
            else:
                for spec in self.dataset.get_list_spectrum_numbers():
                    order_SD_delta.append("SD_delta_" + diluent + "_" + str(spec))
                    param_linelist_df[
                        "SD_delta_" + diluent + "_" + str(spec) + "_vary"
                    ] = len(param_linelist_df) * [False]
                    param_linelist_df[
                        "SD_delta_" + diluent + "_" + str(spec) + "_err"
                    ] = len(param_linelist_df) * [0]
                    if (self.lineprofile == "VP") or (self.lineprofile == "NGP"):
                        param_linelist_df.loc[
                            :, "SD_delta_" + diluent + "_" + str(spec)
                        ] = 0
                    else:
                        param_linelist_df[
                            "SD_delta_" + diluent + "_" + str(spec)
                        ] = param_linelist_df["SD_deltaamma_" + diluent].values
            ...
            param_linelist_df["nuVC_" + diluent + "_vary"] = len(param_linelist_df) * [
                False
            ]
                        ...
                        for molecule in vary_nuVC:
                            for isotope in vary_nuVC[molecule]:
                                param_linelist_df.loc[
                                    ...
                                    & (param_linelist_df["local_iso_id"] == isotope),
                                    "nuVC_" + diluent + "_vary",
                                ] = vary_nuVC[molecule][isotope]
            else:
                for spec in self.dataset.get_list_spectrum_numbers():
                    order_nuVC.append("nuVC_" + diluent + "_" + str(spec))
                    param_linelist_df[
                        "nuVC_" + diluent + "_" + str(spec) + "_vary"
                    ] = len(param_linelist_df) * [False]
                    param_linelist_df[
                        "nuVC_" + diluent + "_" + str(spec) + "_err"
                    ] = len(param_linelist_df) * [0]
                    if (self.lineprofile == "VP") or (self.lineprofile == "SDVP"):
                        param_linelist_df.loc[
                            :, "nuVC_" + diluent + "_" + str(spec)
                        ] = 0
                    else:
                        param_linelist_df[
                            "nuVC_" + diluent + "_" + str(spec)
                        ] = param_linelist_df["nuVC_" + diluent].values
            ...
            param_linelist_df["eta_" + diluent + "_vary"] = len(param_linelist_df) * [
                False
            ]
            ...
            if self.eta_constrain:
                if (
                    (self.lineprofile == "VP")
                    or (self.lineprofile == "SDVP")
                    or (self.lineprofile == "NGP")
                    or (self.lineprofile == "SDNGP")
                ):
                        ...
                        for molecule in vary_eta:
                            for isotope in vary_eta[molecule]:
                                param_linelist_df.loc[
                                    ...
                                    & (param_linelist_df["local_iso_id"] == isotope),
                                    "eta_" + diluent + "_vary",
                                ] = vary_eta[molecule][isotope]
            else:
                for spec in self.dataset.get_list_spectrum_numbers():
                    order_eta.append("eta_" + diluent + "_" + str(spec))
                    param_linelist_df["eta_" + str(spec) + "_vary"] = len(
                        param_linelist_df
                    ) * [False]
                    param_linelist_df["eta_" + str(spec) + "_err"] = len(
                        param_linelist_df
                    ) * [0]
                    if (
                        (self.lineprofile == "VP")
                        ...
                        param_linelist_df[
                            "eta_" + diluent + "_" + str(spec)
                        ] = param_linelist_df["eta_" + diluent].values
                        if vary_eta != {}:
                            for molecule in vary_eta:
                                for isotope in vary_eta[molecule]:
                                    param_linelist_df.loc[
            ...
            for nominal_temp in list_nominal_temps:
                ...
                param_linelist_df[
                    "y_" + diluent + "_" + str(nominal_temp) + "_vary"
                ] = len(param_linelist_df) * [False]
                param_linelist_df[
                    "y_" + diluent + "_" + str(nominal_temp) + "_err"
                ] = len(param_linelist_df) * [0]
                if self.linemixing_constrain:
                    if not self.linemixing:
                        param_linelist_df.loc[
                            :, "y_" + diluent + "_" + str(nominal_temp)
                        ] = 0
                    else:
                        if vary_linemixing != {}:
                            for molecule in vary_linemixing:
                                for isotope in vary_linemixing[molecule]:
                                    param_linelist_df.loc[
                                        ...
                                        + str(nominal_temp)
                                        + "_vary",
                                    ] = vary_linemixing[molecule][isotope]
                else:
                    for spec in self.dataset.get_list_spectrum_numbers():
                        order_linemixing.append(
                            "y_" + diluent + "_" + str(nominal_temp) + "_" + str(spec)
                        )
                        param_linelist_df[
                            "y_"
                            + diluent
                        ...
                        if not self.linemixing:
                            ...
                            param_linelist_df[
                                ...
                                + "_"
                                + str(spec)
                            ] = param_linelist_df["y_" + diluent].values
                            if vary_linemixing != {}:
                                for molecule in vary_linemixing:
                                    for isotope in vary_linemixing[molecule]:
                                        param_linelist_df.loc[
                                            ...
                                            & (param_linelist_df["sw"] > 1)
                                            & (
                                                param_linelist_df["molec_id"]
                                                == molecule
                                            )
                                            & (
                                                param_linelist_df["local_iso_id"]
                                                == isotope
                                            ),
                                            "y_"
                                            + diluent
            ...
            if num_nominal_temps > 1:
                param_linelist_df["n_gamma0_" + diluent + "_vary"] = len(
                    param_linelist_df
                ) * [False]
                param_linelist_df["n_gamma0_" + diluent + "_err"] = len(
                    param_linelist_df
                ) * [0]
                param_linelist_df["n_delta0_" + diluent + "_vary"] = len(
                    param_linelist_df
                ) * [False]
                param_linelist_df["n_delta0_" + diluent + "_err"] = len(
                    param_linelist_df
                ) * [0]
                param_linelist_df["n_gamma2_" + diluent + "_vary"] = len(
                    param_linelist_df
                ) * [False]
                param_linelist_df["n_gamma2_" + diluent + "_err"] = len(
                    param_linelist_df
                ) * [0]
                param_linelist_df["n_delta2_" + diluent + "_vary"] = len(
                    param_linelist_df
                ) * [False]
                param_linelist_df["n_delta2_" + diluent + "_err"] = len(
                    param_linelist_df
                ) * [0]
                param_linelist_df["n_nuVC_" + diluent + "_vary"] = len(
                    param_linelist_df
                ) * [False]
                param_linelist_df["n_nuVC_" + diluent + "_err"] = len(
                    param_linelist_df
                ) * [0]
                # n_Gamma0
                if vary_n_gamma0 != {}:
                    for molecule in vary_n_gamma0:
                        for isotope in vary_n_gamma0[molecule]:
                            param_linelist_df.loc[
                                ...
                                & (param_linelist_df["molec_id"] == molecule)
                                & (param_linelist_df["local_iso_id"] == isotope),
                                "n_gamma0_" + diluent + "_vary",
                            ] = vary_n_gamma0[molecule][isotope]
                # n_Delta0
                if vary_n_delta0 != {}:
                    for molecule in vary_n_delta0:
                        for isotope in vary_n_delta0[molecule]:
                            param_linelist_df.loc[
                                ...
                                & (param_linelist_df["molec_id"] == molecule)
                                & (param_linelist_df["local_iso_id"] == isotope),
                                "n_delta0_" + diluent + "_vary",
                            ] = vary_n_delta0[molecule][isotope]
                # n_Gamma2
                        ...
                        for molecule in vary_n_gamma2:
                            for isotope in vary_n_gamma2[molecule]:
                                param_linelist_df.loc[
                                    ...
                                    & (param_linelist_df["molec_id"] == molecule)
                                    & (param_linelist_df["local_iso_id"] == isotope),
                                    "n_gamma2_" + diluent + "_vary",
                                ] = vary_n_gamma2[molecule][isotope]
                # n_Delta2
                if not (self.lineprofile == "VP") or not (self.lineprofile == "NGP"):
                    if vary_n_delta2 != {}:
                        for molecule in vary_n_delta2:
                            for isotope in vary_n_delta2[molecule]:
                                param_linelist_df.loc[
                                    ...
                                    & (param_linelist_df["molec_id"] == molecule)
                                    & (param_linelist_df["local_iso_id"] == isotope),
                                    "n_delta2_" + diluent + "_vary",
                                ] = vary_n_delta2[molecule][isotope]
                # n_nuVC
                        ...
                        for molecule in vary_n_nuVC:
                            for isotope in vary_n_nuVC[molecule]:
                                param_linelist_df.loc[
                                    ...
                                    & (param_linelist_df["molec_id"] == molecule)
                                    & (param_linelist_df["local_iso_id"] == isotope),
                                    "n_nuVC_" + diluent + "_vary",
                                ] = vary_n_nuVC[molecule][isotope]
    ...
    def generate_fit_baseline_linelist(
        ...
        for param in parameters:
            ...
            if "Pressure" in param:
                base_linelist_df[param + "_vary"] = len(base_linelist_df) * [
                    (vary_pressure)
                ]
                if vary_pressure:
            ...
            if "baseline" in param:
                ...
                base_linelist_df.loc[
                    base_linelist_df["Baseline Order"] >= order, param + "_vary"
                ] = vary_baseline
            if "molefraction" in param:
                for molecule in vary_molefraction:
                    if (ISO[(molecule, 1)][4]) in param:
                        base_linelist_df.loc[
                            base_linelist_df[param] != 0, param + "_vary"
                        ] = vary_molefraction[molecule]
            if "amp" in param:
                base_linelist_df.loc[
                    base_linelist_df[param] != 0, param + "_vary"
                ] = vary_etalon_amp
            ...
            if "phase" in param:
                base_linelist_df.loc[
                    base_linelist_df[param.replace("phase", "freq")] != 0,
                    param + "_vary",
                ] = vary_etalon_phase
...
class Edit_Fit_Param_Files:
    ...
    def edit_generated_baselist(self):
        ...
        baseline_widget = qgrid.show_grid(
            base_linelist_df,
            grid_options={"forceFitColumns": False, "defaultColumnWidth": 200},
        )
...
class Fit_DataSet:
    ...
    def __init__(
        ...
        pressure_limit=False,
        pressure_limit_factor=10,
        temperature_limit=False,
        temperature_limit_factor=10,
        molefraction_limit=False,
        molefraction_limit_factor=10,
        etalon_limit=False,
        etalon_limit_factor=50,  # phase is constrained to +/- 2pi,
        x_shift_limit=False,
        x_shift_limit_magnitude=0.1,
        nu_limit=False,
        nu_limit_magnitude=0.1,
        sw_limit=False,
        sw_limit_factor=10,
        gamma0_limit=False,
        gamma0_limit_factor=10,
        n_gamma0_limit=True,
        n_gamma0_limit_factor=10,
        delta0_limit=False,
        delta0_limit_factor=10,
        n_delta0_limit=True,
        n_delta0_limit_factor=10,
        SD_gamma_limit=False,
        SD_gamma_limit_factor=10,
        n_gamma2_limit=True,
        n_gamma2_limit_factor=10,
        SD_delta_limit=True,
        SD_delta_limit_factor=10,
        n_delta2_limit=True,
        n_delta2_limit_factor=10,
        nuVC_limit=False,
        nuVC_limit_factor=10,
        n_nuVC_limit=True,
        n_nuVC_limit_factor=10,
        eta_limit=True,
        eta_limit_factor=10,
        linemixing_limit=False,
        linemixing_limit_factor=10,
    ):
    ...
    def generate_params(self):
        ...
        for base_param in list(self.baseline_list):
            if (
                ...
                and ("Spectrum Number" not in base_param)
        ...
        for index in self.baseline_list.index.values:
            ...
            for base_param in baseline_parameters:
                if self.baseline_list.loc[index][base_param] == 0:
                    ...
                    and self.etalon_limit
                    and ("phase" not in base_param)
                ):
                    params.add(
                        base_param + "_" + str(int(spec_num)) + "_" + str(int(seg_num)),
                        self.baseline_list.loc[index][base_param],
        ...
        for line_param in list(self.lineparam_list):
            if (self.dataset.get_number_nominal_temperatures()[0]) == 1:
                if (
                    ...
                    and ("_err" not in line_param)
                    and (line_param != "molec_id")
                    and (line_param != "local_iso_id")
                    and (line_param != "elower")
                    and ("n_" not in line_param)
                ):
        ...
        if (
            (sum(("nu" in param) & ("nuVC" not in param) for param in linelist_params))
        ) > 1:
            nu_constrain = False
        if (sum("sw" in param for param in linelist_params)) > 2:
            sw_constrain = False
        if (self.dataset.get_number_nominal_temperatures()[0]) == 1:
            if (sum("gamma0" in param for param in linelist_params)) > len(
                diluent_list
            ):
                gamma0_constrain = False
            if (sum("delta0" in param for param in linelist_params)) > len(
                diluent_list
            ):
                delta0_constrain = False
            if (sum("SD_gamma" in param for param in linelist_params)) > len(
                diluent_list
            ):
                SD_gamma_constrain = False
            if (sum("SD_delta" in param for param in linelist_params)) > len(
                diluent_list
            ):
            ...
            if (sum("gamma0" in param for param in linelist_params)) > 2 * len(
                diluent_list
            ):
                gamma0_constrain = False
            if (sum("delta0" in param for param in linelist_params)) > 2 * len(
                diluent_list
            ):
                delta0_constrain = False
            if (sum("SD_gamma" in param for param in linelist_params)) > len(
            ...
            if (sum("SD_delta" in param for param in linelist_params)) > len(
                ...
                SD_delta_constrain = False
            if (sum("nuVC" in param for param in linelist_params)) > 2 * len(
                diluent_list
            ):
                nuVC_constrain = False
            if (sum("eta" in param for param in linelist_params)) > len(diluent_list):
        ...
        if (sum("y" in param for param in linelist_params)) > len(diluent_list) * (
            self.dataset.get_number_nominal_temperatures()[0]
        ):
            ...
            if (
                ...
                for line_param in linelist_params:
                    ...
                    if line_param == "nu" and nu_constrain:
                        ...
                        (line_param != "nu")
                        and ("nu" in line_param)
                        and ("nuVC" not in line_param)
                        and (not nu_constrain)
                    ):
                        if self.nu_limit:
                            params.add(
                                line_param + "_" + "line_" + str(spec_line),
                    ...
                    # SW
                        ...
                        (line_param != "sw")
                        and ("sw" in line_param)
                        and (not sw_constrain)
                        and (line_param != "sw_scale_factor")
                    ):
                        if self.sw_limit:
                            params.add(
                                line_param + "_" + "line_" + str(spec_line),
                    ...
                    # GAMMA0
                        ...
                        ("gamma0_" in line_param)
                        and ("n_" not in line_param)
                        and (gamma0_constrain)
                        and (index_length == 1)
                    ):
                        if (
                            self.gamma0_limit
                    ...
                    # DELTA0
                        ...
                        ("delta0" in line_param)
                        and ("n_" not in line_param)
                        and (delta0_constrain)
                        and (index_length == 1)
                    ):
                        if (
                            self.delta0_limit
                        ...
                        ("delta0_" in line_param)
                        and ("n_" not in line_param)
                        and (not delta0_constrain)
                        and (index_length > 1)
                    ):
                        if (
                            self.delta0_limit
                    ...
                    # SD Gamma
                        ...
                        ("SD_gamma" in line_param)
                        and (SD_gamma_constrain)
                        and (index_length == 2)
                    ):
                        if (
                            self.SD_gamma_limit
                    ...
                    # SD Delta
                        ...
                        ("SD_delta" in line_param)
                        and (SD_delta_constrain)
                        and (index_length == 2)
                    ):
                        if (
                            self.SD_delta_limit
                    ...
                    # nuVC
                        ...
                        ("nuVC" in line_param)
                        and ("n_nuVC" not in line_param)
                        and (not nuVC_constrain)
                        and (index_length > 1)
                    ):
                        if (
                            self.nuVC_limit
                    ...
                    # linemixing
                        ...
                        ("y_" in line_param)
                        and (linemix_constrain)
                        and (line_param in linemix_terms_constrained)
                    ):
                        if (
                        ...
                        ("y_" in line_param)
                        and (not linemix_constrain)
                        and (line_param not in linemix_terms_constrained)
                    ):
                        if (
                            self.linemixing_limit
                            and self.lineparam_list.loc[spec_line][line_param] != 0
    ...
    def constrained_baseline(
        self,
        params,
        baseline_segment_constrained=True,
        xshift_segment_constrained=True,
        molefraction_segment_constrained=True,
        etalon_amp_segment_constrained=True,
        etalon_freq_segment_constrained=True,
        etalon_phase_segment_constrained=True,
        pressure_segment_constrained=True,
        temperature_segment_constrained=True,
    ):
        ...
        for spectrum in self.dataset.spectra:
            spectrum_segment_min[spectrum.spectrum_number] = np.min(
                list(set(spectrum.segments))
            )
        ...
        for param in params:
            if ("Pressure" in param) and pressure_segment_constrained:
                ...
                if segment_num != spectrum_segment_min[spectrum_num]:
                    params[param].set(
                        expr=param[: indices[1] + 1]
                        + str(spectrum_num)
                        + "_"
                        + str(spectrum_segment_min[spectrum_num])
                    )
    ...
    def simulation_model(
        self, params, wing_cutoff=50, wing_wavenumbers=50, wing_method="wing_cutoff"
    ):
        ...
        for param in list(params.valuesdict().keys()):
            if (
                ("molefraction" in param)
                or ("baseline" in param)
                or ("etalon" in param)
                or ("x_shift" in param)
                or ("Pressure" in param)
                or ("Temperature" in param)
            ):
        ...
        for spectrum in self.dataset.spectra:
            ...
            for segment in list(set(spectrum.segments)):
                ...
                # Simulate Spectra
                fit_nu, fit_coef = HTP_from_DF_select(
                    linelist_for_sim,
                    wavenumbers,
                ...
                for param in baseline_params:
                    if "baseline" in param:
                        ...
                        spectrum_num = int(param[indices[1] + 1 : indices[2]])
                        segment_num = int(param[indices[2] + 1 :])
                        if (spectrum_num == spectrum_number) and (
                            segment_num == segment
                        ):
                            baseline_param_array[
                                ord(param[9 : param.find("_", 9)]) - 97
                            ] = np.float(params[param])
                baseline_param_array = baseline_param_array[
                    ::-1
                ]  # reverses array to be used for polyval
                ...
                for param in baseline_params:
                    if ("etalon" in param) and (
                        str(spectrum_number) in param[param.find("_", 7) :]
                    ):
                        etalon_num = int(
                            param[
                                param.find("_")
                                + 1 : param.find("_", param.find("_") + 1)
                            ]
                        )
                        if param == "etalon_" + str(etalon_num) + "_amp_" + str(
                            spectrum_number
                        ) + "_" + str(
                            segment
                        ):  # ('amp' in param) and (str(etalon_num) in param):
                            fit_etalon_parameters[etalon_num]["amp"] = np.float(
                                params[param]
                            )
                        if param == "etalon_" + str(etalon_num) + "_freq_" + str(
                            spectrum_number
                        ) + "_" + str(
                            segment
                        ):  # ('freq' in param) and (str(etalon_num) in param):
                            fit_etalon_parameters[etalon_num]["freq"] = np.float(
                                params[param]
                            )
                        if param == "etalon_" + str(etalon_num) + "_phase_" + str(
                            spectrum_number
                        ) + "_" + str(
                            segment
                        ):  # ('phase' in param) and (str(etalon_num) in param):
                            fit_etalon_parameters[etalon_num]["phase"] = np.float(
                                params[param]
                            )
                etalons = len(wavenumbers) * [0]
                for i in range(1, len(spectrum.etalons) + 1):
                    etalons += etalon(
                        wavenumbers_relative,
                        fit_etalon_parameters[i]["amp"],
                        fit_etalon_parameters[i]["freq"],
                        fit_etalon_parameters[i]["phase"],
                    )
                simulated_spectra[
                    np.min(indices_segments[segment]) : np.max(
                        indices_segments[segment]
                    )
                    + 1
                ] = (baseline + etalons + fit_coef)
                residuals[
                    np.min(indices_segments[segment]) : np.max(
                        indices_segments[segment]
                    )
                    + 1
                ] = (baseline + etalons + fit_coef) - alpha_segments[segment]
    ...
    def residual_analysis(self, result, indv_resid_plot=False):
        ...
        for spectrum in self.dataset.spectra:
            spectrum_residual, residual_array = np.split(
                residual_array, [len(spectrum.wavenumber)]
            )
    ...
    def update_params(
        ...
        for key, par in result.params.items():
            if ("Pressure" in par.name) or ("Temperature" in par.name):
                ...
                ("molefraction" in par.name)
                or ("baseline" in par.name)
                or ("x_shift" in par.name)
            ):
                indices = [m.start() for m in re.finditer("_", par.name)]
        ...
        for spectrum in self.dataset.spectra:
            ...
            for segment in set(spectrum.segments):
                ...
                for i in range(1, len(spectrum.etalons) + 1):
                    fit_etalon_parameters[i] = {"amp": 0, "freq": 1, "phase": 0}
                for key, par in result.params.items():
                    if ("baseline" in par.name) and (
                        str(spectrum.spectrum_number) in par.name
                    ):
                        baseline_param_array[
                            ord(par.name[9 : par.name.find("_", 9)]) - 97
                        ] = np.float(par.value)
                    elif ("etalon" in par.name) and (
                        str(spectrum.spectrum_number)
                        in par.name[par.name.find("_", 7) :]
                    ):
                        etalon_num = int(
                            par.name[
                                par.name.find("_")
                                + 1 : par.name.find("_", par.name.find("_") + 1)
                            ]
                        )
                ...
                baseline[bound_min : bound_max + 1] += np.polyval(
                    baseline_param_array, wave_rel
                )
                for i in range(1, len(spectrum.etalons) + 1):
                    baseline[bound_min : bound_max + 1] += etalon(
                        wave_rel,
                        fit_etalon_parameters[i]["amp"],
                        fit_etalon_parameters[i]["freq"],
                        fit_etalon_parameters[i]["phase"],
